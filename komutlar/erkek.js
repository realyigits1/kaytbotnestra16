...
if(member.id === message.author.id) return message.channel.send('<a:YanpSnennleGif:810876071842938900> Kendini kayıt edemezsin. <a:YanpSnennleGif:810876071842938900>')
if(member.id === client.user.id) return message.channel.send('<a:YanpSnennleGif:810876071842938900> Botu kayıt edemezsin. <a:YanpSnennleGif:810876071842938900>')
if(member.id === message.guild.OwnerID) return message.channel.send('<a:YanpSnennleGif:810876071842938900> Sunucu sahibini kayıt edemezsin. <a:YanpSnennleGif:810876071842938900>')
...
if(!args[0]) return message.channel.send('<a:YanpSnennleGif:810876071842938900> Bir kullanıcı belirt <a:YanpSnennleGif:810876071842938900>')  
...
.setTitle("Kayıt Başarılı <a:OnaylamakGif:810876076427444284>")
.setThumbnail(member.user.avatarURL({dynamic: true}))
.setDescription(`
 **Kayıt Edilen kullanıcı : ${member} <:ndirmePng:810875993597149214>
 Kayıt İşleminde Verilen Rol : <@&${ayarlar.erkekROL}>
 Yeni Kullanıcı Adı :  \`${tag} ${name} | ${age}\` ** <:KitapPng:810875993232244738>

 **Bu Kullanıcının Sunucudaki Eski İsimleri <a:donenlogo:818048722667438101> [${sayı++ || "0"}]     \n\n${data.join('\n')} ** `).setColor('0x348f36').setFooter(message.author.tag, message.author.avatarURL({dynamic: true})).setTimestamp()  
